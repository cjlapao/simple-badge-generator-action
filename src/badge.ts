...
export class Badge {
    ...
    if (this.valueFillColor == "") {
      switch (this.type) {
        ...
        case "cobertura":
          ...
          } else if (parseFloat(this.value) >= this.coberturaYellowThreshold && parseFloat(this.value) < this.coberturaGreenThreshold) {
          ...
          break;
        case "semaphore":
          if (this.value.toLocaleLowerCase() === this.semaphoreRedValue.toLocaleLowerCase()) {
            this.valueFillColor = redColor;
            if (this.semaphoreRedColor) {
              this.valueFillColor = this.semaphoreRedColor;
            }
          } else if (this.value.toLocaleLowerCase() === this.semaphoreYellowValue.toLocaleLowerCase()) {
            this.valueFillColor = yellowColor;
            if (this.semaphoreYellowColor) {
              this.valueFillColor = this.semaphoreYellowColor;
            }

          } else if (this.value.toLocaleLowerCase() === this.semaphoreGreenValue.toLocaleLowerCase()) {
            this.valueFillColor = greenColor;
  ...
  interpolateColor(color1: string, color2: string, steps: number): string[] {
    const colorArray1: number[] = color1.match(/\w\w/g)!.map((c) => parseInt(c, 16));
    const colorArray2: number[] = color2.match(/\w\w/g)!.map((c) => parseInt(c, 16));
    const interpolatedColors: string[] = [];

    for (let step = 0; step <= steps; step++) {
      const interpolatedColor = colorArray1.map((startValue, i) => {
        const endValue = colorArray2[i];
        const stepValue = startValue + ((endValue - startValue) * (step / steps));
